...
func update_texture(mode: int) -> void:
	if mode == Globals.Mode.MOUSE:
		change_key_texture(InputMap.action_get_events(action_name)[0].as_text())
	else:
		change_key_texture(Globals.get_joypad_event_image_id(InputMap.action_get_events(action_name)[1]))


func change_key_texture(text: String) -> void:
	# print(text)
