...
func _ready() -> void:
	...
	for cell in tilemap.get_used_cells(0):
		if tilemap.get_cell_atlas_coords(0, cell) in FLOOR_TILES_ATLAS_COOR:
			floor_pos.push_back(room.position + tilemap.map_to_local(cell))
...
#	var rand_hole2: Vector2 = holes_array[randi() % holes_array.size()]
	...
	global_position = floor_pos[randi() % floor_pos.size()]
