...
func add() -> void:
	life_component.take_damage(1, Vector2.ZERO, 0, null, "lightning")
	...
	for enemy: Enemy in get_tree().get_nodes_in_group("enemies"):
		...
		if dis <= EFFECT_RANGE:
			enemy.life_component.take_damage(1, (enemy.global_position - character.global_position).normalized(), 300, null, "lightning")
