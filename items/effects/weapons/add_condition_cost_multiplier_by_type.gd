...
func get_description() -> String:
	return _get_color_tag(GREEN if _amount < 0 else RED) % (tr("WEAPON_CONDITION_COST_MULTIPLIER_BY_TYPE") % [_number_to_string(_amount * 100), tr(WeaponData.Type.keys()[_type].to_snake_case().to_upper())])
