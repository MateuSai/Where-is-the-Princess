class_name ArmorAbilityBar extends TextureProgressBar
...
func _on_armor_changed(new_armor: Armor) -> void:
	...
	texture_under = new_armor.ability_icon
	texture_progress = new_armor.ability_icon
	value = 100
...
func _on_armor_ability_effect_ended() -> void:
	_tween = create_tween()
	_tween.tween_property(self, "value", 100, player.get_armor_recharge_time())
	await _tween.finished
	var shine_effect: Sprite2D = ARMOR_SHINE_EFFECT.instantiate()
	#shine_effect.position = Vector2(0, 0)
